...
struct ForecastCard: View {
    ...
    var isActive: Bool {
        if forecastPeriod == .hourly {
            let isThisHour = Calendar.current.isDate(.now, equalTo: forecast.date,
                                                 toGranularity: .hour)
            return isThisHour
        } else {
            let isToday = Calendar.current.isDate(.now, equalTo: forecast.date,
                                                  toGranularity: .day)
            return isToday
        }
    }
        ...
        ZStack {
            RoundedRectangle(cornerRadius: 30)
                ...
                .innerShadow(shape: RoundedRectangle(cornerRadius: 30),
                             ...
                             offsetX: 1, offsetY: 1, blur: 0, blendMode: .overlay)
            VStack(spacing: 16) {
                Text(forecast.date, format: forecastPeriod == .hourly
                     ? .dateTime.hour()
                     : .dateTime.weekday())
